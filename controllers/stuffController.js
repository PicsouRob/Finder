...
module.exports = {
    ...
    getOneUserStuff: (req, res) => {
        ...
        Job.find({ creatorId: '617c831ccf42000023fb11f1' }).then(response => {
            if(!response) {
                return res.send({ 
                    error: "Oups ! désolé, aucun résultat trouvé"
                });
...
//         req.files === undefined ? null :
//         req.files.forEach((ele) => {
//             imagesArray.push(`https://finderht.herokuapp.com/userProfil/${ele.filename}`);
//         });
        
//         job.updateOne({ location, nameCreator, email, phone, description,
...
//             res.status(200).json({ message: "Objet modified !", success, file: req.files });
//         })
