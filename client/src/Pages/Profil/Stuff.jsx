...
function Stuff(props) {
    const { item, index, setShowModal, showModal, setModalData } = props;
    ...
    return (
        <div class="flex item-center justify-between rounded-lg shadow-md px-3 py-4 cursor-pointer group hover:bg-black"
            key={index}
        ...
        >
            <div class="flex item-center gap-x-4">
                ...
                />
                <div class="">
                    <span class="group-hover:text-white">{item.job}</span>
                    <p class="">{substringDesc(item.description)}</p>
                    <p $date>DEPUIS {getDate(item.date)}</p>
                </div>
            </div>
